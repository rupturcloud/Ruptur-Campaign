...
export function UazapiConfig({ value, onChange, showPreview = true }: { value: any; onChange: (value: any) => void; showPreview?: boolean }) {
  ...
  <div className="space-y-4">
    ...
    {['button', 'list', 'poll'].includes(c.type) && <>
      {c.type === 'button' ? <div className="space-y-2"><p className="text-sm font-medium text-gray-700">Botões da mensagem</p>{(c.buttons || []).map((button: any, index: number) => <div key={index} className="grid grid-cols-[1fr_120px_1fr_auto] gap-2 rounded border p-2"><input className={style} placeholder="Texto" value={button.text || ''} onChange={e => { const buttons = [...c.buttons]; buttons[index] = { ...button, text: e.target.value }; set('buttons', buttons); }} /><select className={style} value={button.kind || 'reply'} onChange={e => { const buttons = [...c.buttons]; buttons[index] = { ...button, kind: e.target.value, value: e.target.value === 'url' ? 'https://' : '' }; set('buttons', buttons); }}><option value="reply">Resposta</option><option value="url">Link</option><option value="copy">Copiar</option></select><input className={style} placeholder={button.kind === 'url' ? 'https://...' : 'Destino'} value={button.value || ''} onChange={e => { const buttons = [...c.buttons]; buttons[index] = { ...button, value: e.target.value }; set('buttons', buttons); }} /><button type="button" className="text-red-600" onClick={() => set('buttons', c.buttons.filter((_: any, i: number) => i !== index))}>×</button></div>)}<button type="button" className="text-sm font-medium text-[#008069]" onClick={() => set('buttons', [...(c.buttons || []), { kind: 'reply', text: '', value: '' }])}>+ Adicionar botão</button><p className="text-xs text-gray-500">Arraste os botões diretamente no preview para alterar sua ordem. Botões de link usam URL HTTPS.</p></div> : <><label className="block text-sm">Opções (uma por linha)<textarea className={style} rows={5} value={(c.choices || []).join('\n')} onChange={e => set('choices', e.target.value.split('\n'))} /></label><p className="text-xs text-gray-500">Listas: Texto|identificador|descrição; use [Seção] para agrupar opções.</p></>}
